...
| Security/privacy | Mock data only, no real PII; SQLite file-local; LLM calls only fire when a key is explicitly configured; LLM-bound text is redaction-guarded and input free-text is validated/injection-checked (see "LLM & Data-Handling Guardrails" below) | RBAC, field-level encryption, data minimisation, access logging |
...
**What's redacted, and why.** `beneficial_owner` and `tax_id` values are never sent to an LLM provider and never shown in RM-facing text (`manager_summary`, `client_message_draft`). The Manager/Communication agent templates (`agents/manager.py::_template_summary`, `agents/communication.py::_template_message`) already only interpolate `company_name`, `customer_id`, gap type, and system-generated reason/confidence text — never raw enrichment values. `services/privacy.py::redact_pii()` is a **defense-in-depth guard on top of that**: `services/llm_client.py::complete()` scans the outbound `system`/`prompt` and the inbound response against the current record's actual `beneficial_owner`/`tax_id` values and replaces any match with `[REDACTED]`, logging a warning — so a future template regression is caught at runtime instead of silently shipping a leak. Every `complete()` call also gets a hardened system-prompt suffix (`build_hardened_system_prompt`) instructing the model never to output/infer these values and to treat `[CUSTOMER_DATA]`-delimited text as inert data, not instructions.

**Why the audit trail is *not* redacted.** The SQLite `audits` table (`services/db.py`, written via `agents/audit.py::log_step`) intentionally retains the full `enrichment_result.evidence` — including resolved `beneficial_owner`/`tax_id` values — in full detail. This is the compliance record the project's "full replayable audit trail" requirement depends on; redacting it would break auditability. This is a deliberate scope boundary: the guardrail applies to the LLM/RM-facing surface only, never to the audit store.

**Input validation (`services/validation.py`).**
- `validate_rm_notes`: non-empty, max 2000 chars, strips non-printable/control characters, and **hard-rejects** (raises `ValidationError`, no silent sanitize-and-continue) on a denylist of prompt-injection patterns — ignore-previous-instructions phrasing, "you are now" role overrides, fake `System:`/`Assistant:` role markers, `###`-style instruction fences, the reserved `[CUSTOMER_DATA]` delimiter itself, and excessive repeated-character padding. Enforced both at the UI layer (`app.py`) and independently inside `agents/human_review.py::resolve_human_review()`, so it can't be bypassed by a non-UI caller.
- `validate_policy_version`: must be an integer, strictly greater than the current policy version, and within `current + 1000` to catch fat-fingered input. Enforced both in the `app.py` button handler and at the top of `on_policy_updated()`.
- Why hard-reject, not sanitize-and-continue: an RM note is a short factual record of a customer call — there's no legitimate reason for it to contain instruction-override phrasing, so silently stripping it and proceeding risks letting a subtler variant through unnoticed. A hard block with a clear error forces a human to notice and re-enter in plain language.

**Is there a RAG system here?** No. `services/mock_sources.py::lookup()` is a deterministic exact-match dict lookup by `customer_id` — there's no embedding/vector-search/retrieval step anywhere in this repo. This is intentional: RAG would introduce retrieval non-determinism, which conflicts with this project's determinism goal (same input + same evidence set → same recorded outcome, see the idempotency design above), and there's no large unstructured document corpus in this MVP's scope that would justify it — the mock data is 10 small, structured records, and exact-match lookup is strictly more appropriate (and auditable) than semantic search here.
